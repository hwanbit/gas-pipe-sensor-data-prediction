...
# 슬라이딩 윈도우 파라미터 설정
window_size = 8  # 초 동안의 데이터 사용
target_step = 10  # 초 뒤의 데이터 예측
...
model_checkpoint = ModelCheckpoint(
    filepath='best_model_10sec.h5',  # 모델 저장 경로
...
# 저장된 모델의 최적 epoch 확인
if os.path.exists('best_model_10sec.h5'):
...
history_dict = history_10sec.history  # loss와 dict 형태로 저장되어 있음
print(history_dict.keys())

with open('training_history_10sec.pkl', 'wb') as f:
...
plt.plot(history_10sec.history['loss'], label='Loss')
plt.plot(history_10sec.history['mae'], label='MAE')
...
np.savez('predictions10sec_and_labels.npz',
         predictions_10sec=predictions,
         y_test=y_test)
