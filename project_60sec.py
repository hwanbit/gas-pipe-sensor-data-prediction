...
# 슬라이딩 윈도우 파라미터 설정
window_size = 40  # 초 동안의 데이터 사용
target_step = 60  # 초 뒤의 데이터 예측
...
model_checkpoint = ModelCheckpoint(
    filepath='best_model_60sec.h5',  # 모델 저장 경로
...
# 저장된 모델의 최적 epoch 확인
if os.path.exists('best_model_60sec.h5'):
...
history_dict = history_60sec.history  # loss와 dict 형태로 저장되어 있음
print(history_dict.keys())

with open('training_history_60sec.pkl', 'wb') as f:
...
plt.plot(history_60sec.history['loss'], label='Loss')
plt.plot(history_60sec.history['mae'], label='MAE')
...
np.savez('predictions60sec_and_labels.npz',
         predictions_60sec=predictions,
         y_test=y_test)
