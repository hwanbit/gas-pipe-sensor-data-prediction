...
model_checkpoint = ModelCheckpoint(
    filepath='best_model_6sec.h5',  # 모델 저장 경로
...
# 저장된 모델의 최적 epoch 확인
if os.path.exists('best_model_6sec.h5'):
...
history_dict = history_6sec.history  # loss와 dict 형태로 저장되어 있음
print(history_dict.keys())

with open('training_history_6sec.pkl', 'wb') as f:
...
plt.plot(history_6sec.history['loss'], label='Loss')
plt.plot(history_6sec.history['mae'], label='MAE')
...
np.savez('predictions6sec_and_labels.npz',
         predictions_6sec=predictions,
         y_test=y_test)
