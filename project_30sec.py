...
# 슬라이딩 윈도우 파라미터 설정
window_size = 25  # 초 동안의 데이터 사용
target_step = 30  # 초 뒤의 데이터 예측
...
for file in selected_files:
    ...
    # 필요하다면 여기서 X와 y를 추가 처리하거나 저장할 수 있습니다.
...
model_checkpoint = ModelCheckpoint(
    filepath='best_model_30sec.h5',  # 모델 저장 경로
...
# 저장된 모델의 최적 epoch 확인
if os.path.exists('best_model_30sec.h5'):
...
history_dict = history_30sec.history  # loss와 dict 형태로 저장되어 있음
print(history_dict.keys())

with open('training_history_30sec.pkl', 'wb') as f:
...
plt.plot(history_30sec.history['loss'], label='Loss')
plt.plot(history_30sec.history['mae'], label='MAE')
...
np.savez('predictions30sec_and_labels.npz',
         predictions_30sec=predictions,
         y_test=y_test)
